...
//	alert('came in RecentO.ready()');
	...
	function selBtnEvent() {
		for (var i = 0; i < MAX_ROWS; i++) {
			if (tIDs[i][2] == caller.id) {
				setCookie('orderID', String(tVal[i][0]));
				alert('working...'+i);
				break;
			}
		}
	...
	function setIDs(rows) {
		...
		//	tIDs[i][2].addEventListener('click', selBtnEvent());
		}
	}
	...
	function callServlet() {
		...
		$.ajax({                       // Performs all the async comm. part (Client <-> Server)
			...
			url: './OrderMgmtServer',   	// Servlet name
